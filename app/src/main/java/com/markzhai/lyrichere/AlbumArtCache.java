...
import com.markzhai.lyrichere.utils.BitmapHelper;
...
public final class AlbumArtCache {
    private static final String TAG = LogUtils.makeLogTag(AlbumArtCache.class);
    ...
    public void fetch(final String artUrl, final FetchListener listener) {
        ...
        if (bitmap != null) {
            LogUtils.d(TAG, "getOrFetch: album art is in cache, using it", artUrl);
            listener.onFetched(artUrl, bitmap[BIG_BITMAP_INDEX], bitmap[ICON_BITMAP_INDEX]);
            return;
        }
        LogUtils.d(TAG, "getOrFetch: starting asynctask to fetch ", artUrl);
        ...
        new AsyncTask<Void, Void, Bitmap[]>() {
            ...
            protected Bitmap[] doInBackground(Void[] objects) {
                ...
                LogUtils.d(TAG, "doInBackground: putting bitmap in cache. cache size=" +
                        mCache.size());
                return bitmaps;
            }
    ...
    public static abstract class FetchListener {
        ...
        public void onError(String artUrl, Exception e) {
            LogUtils.e(TAG, e, "AlbumArtFetchListener: error while downloading " + artUrl);
        }
    }
}
